...
import { SerSanitizedFiltered } from "./ser.filter.mjs";
import type {
  SerNodeFiltersRecord,
  SerNodeSanitizedTypesRecord,
} from "./ser.filter.types.mjs";
 ...
 * @aidoc
 */
export function createFilteredSer(
  parsed: SanitizedParseResult,
  filters: SerNodeFiltersRecord,
): SanitizeModes<Theatered<SerNodeSanitizedTypesRecord[]>[]> {
  return filterCommon<SerNodeSanitizedTypesRecord[]>(
    parsed,
    // FIX
    //  @ts-expect-error doesn't account for returning an array
    (success) => {
      return success.ser.map((p) => {
        return {
          theater: p.theater,
          sanitized: p.serialized.map((s) =>
            new SerSanitizedFiltered(s, filters).build(),
          ),

          // sanitized: p.tCpx.ser.map((t) =>
          //   new SerSanitizedFiltered(t, filters).build(),
          // ),
        };
      });
    },
