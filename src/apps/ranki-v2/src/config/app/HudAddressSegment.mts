...
export class HudAddressSegmentConfig {
  public static build({
  ...
  }: AppConfigBuildParams): HudAddressSegment[] {
    ...
    segments.forEach(({ end, mode, start }) => {
      const si = this.translateMarker(parts, start);
      const ei = this.translateMarker(parts, end);
      for (let pi = si; pi < ei; pi++) {
        const mi = MUTATION_MODE_PRECEDENCE.indexOf(mode);
        assertExists(mi, {
          details: { deck, end, mode, segments, start },
    ...
    return this.alt(partMode, tokens, parts);
  }
  ...
  private static translateMarker(
    parts: AnkiDeckParts,
