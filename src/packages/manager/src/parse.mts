...
export function parse(context: ParseContext, raw: string) {
  ...
  if (!parserPlugins.count()) {
    throw new Error("NO ENABLED PLUGINS");
  }

  const activePluginNames = new Set([
    ...context.config.plugins.standards,
    ...context.config.plugins.requested,
  ]);

  parserPlugins.checkPluginSet(activePluginNames);
  ...
  return {
    report: {
      ...
      parser: {
        requested: context.config.plugins.requested,
        importChain,
        dependencyGraph,
      },
    },
