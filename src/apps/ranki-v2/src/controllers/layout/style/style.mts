import type { AnimatableStylesPartial } from "./style.types.mts";
...
export class Style {
  public static produceKeyframe({
    ...
    skewY,
    // rotate3d,
  }: AnimatableStylesPartial): Keyframe {
    const k: Keyframe = {};
    const transform = [
    ...
    if (scale !== undefined) k.scale = scale.toString();
    if (opacity !== undefined) k.opacity = opacity.toString();
